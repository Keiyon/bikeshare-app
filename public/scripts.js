...
function showPosition(position) {
    ...
    $.getJSON("https://api.coord.co/v1/sv/location?latitude=38.995719&longitude=-77.038405&radius_km=100&access_key=lo_qtNC8fHHmJOTkvZnPSv3-aWFdker4UEn5z4LGqzU", function (data) {
                ...
                //         if (bikeData.system_id !== "CapitalBikeShare") {
                //             L.geoJson(jsonData, { //dockless bikes first

                //                 onEachFeature: function (feature, layer) {
                //                     layer.bindPopup(`<p>Company: </p> ${bikeData.system_id} <br><p>Location: </p>  ${bikeData.lon} <p>, </p> ${bikeData.lat} <br> <p>Bikes Available: </p> ${bikeData.num_bikes_available}`) //Return lat and long for dockless bikes
                //                 }
                //             }).addTo(map);
                ...
                //             L.geoJson(jsonData, {
                //                 onEachFeature: function (feature, layer) {
                //                     layer.bindPopup(`<p>Company: </p> ${bikeData.system_id} <br><p>Location: </p> ${bikeData.name} <p> <br> <p>Bikes Available: </p> ${bikeData.num_bikes_available} <p>Number of Docks Avail: </p> ${bikeData.num_docks_available}`)
                //                 }
                //             }).addTo(map);
                ...
                var url = "https://api.coord.co/v1/sv/location?latitude=38.995719&longitude=-77.038405&radius_km=100&access_key=lo_qtNC8fHHmJOTkvZnPSv3-aWFdker4UEn5z4LGqzU"; mymap.on("load", function () {
